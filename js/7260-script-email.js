function initContactButton() { "computer" == deviceName ? sendEmailDiv.onclick = function() { sendEmail() } : sendEmailDiv.addEventListener("touchstart", sendEmail, !1) }
...
function send(a) {
    $.ajax({
        type: "POST",
        url: "https://rushikeshchavan.ezyro.com/",
        data: a,
        cache: !1,
        dataType: "json"
    }).done(function() { hideContactConfirmationContainer(), positionContactConfirmationContainer(), setTimeout("showContactConfirmationContainer(4)", 200), clearAllInputField() }).fail(function() { hideContactConfirmationContainer(), positionContactConfirmationContainer(), setTimeout("showContactConfirmationContainer(3)", 200) })
}
